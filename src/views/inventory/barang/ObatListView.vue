<template>
  <div class="content">
    ...
    <div class="barang-hero mb-2">
      ...
      <Button icon="pi pi-plus" label="Tambah Obat/BMHP" class="round-button2 btn-primary-barang" @click="openForm()" />
    </div>
    ...
    <div class="barang-stat-strip">
      ...
      <div class="strip-stat">
        ...
        <span class="strip-num" style="color:#0f4c81">{{ formatCurrency(totalPersediaanDitampilkan) }}</span>
    ...
    <div class="barang-filter-bar">
      ...
      <div class="barang-filter-group">
        ...
        <Select v-model="filters.JENIS" :options="jenisOptions" optionLabel="label" optionValue="value" placeholder="Semua" style="min-width:140px" @change="onFilterChange" :disabled="loading" />
      </div>
      <div class="filter-vdiv"></div>
      <div class="barang-filter-group">
        <span class="barang-filter-label"><i class="pi pi-sliders-h"></i>Tampilan</span>
        <div class="filter-chip-row">
          <button type="button" class="filter-chip" :class="{ active: expandAll }" @click="expandAll = !expandAll; onToggleExpand()" :disabled="loading || showExpired">
            <i class="pi pi-list"></i>Expand Semua
          </button>
          <button type="button" class="filter-chip" :class="{ active: isGrouped }" @click="isGrouped = !isGrouped; onToggleGrouped()" :disabled="loading || showExpired">
            <i class="pi pi-sitemap"></i>Kelompokkan
          </button>
          <button type="button" class="filter-chip" :class="{ 'active-warn': showBelowMin }" @click="showBelowMin = !showBelowMin; if(showBelowMin) showExpired = false; filters.currentPage = 1; fetchBarang()" :disabled="loading">
      ...
      <div class="barang-filter-group">
        ...
        <div class="filter-chip-row">
          <button type="button" class="filter-chip" :class="{ 'active-expired': showExpired }" @click="showExpired = !showExpired; if(showExpired) showBelowMin = false; filters.currentPage = 1; fetchBarang()" :disabled="loading">
            <i class="pi pi-clock"></i>{{ showExpired ? 'Aktif' : 'Nonaktif' }}
          </button>
          <template v-if="showExpired">
            <div class="filter-vdiv" style="height:28px"></div>
            <button type="button" class="filter-chip" :class="{ active: expiredMonths===1 }" @click="expiredMonths=1; filters.currentPage=1; fetchBarang()" :disabled="loading">1 Bln</button>
            <button type="button" class="filter-chip" :class="{ active: expiredMonths===3 }" @click="expiredMonths=3; filters.currentPage=1; fetchBarang()" :disabled="loading">3 Bln</button>
          </template>
        </div>
      </div>
    ...
    <div class="barang-table-wrap">
      ...
      <!-- ── EXPIRED MODE ──────────────────────────────────────────────── -->
      <DataTable v-if="showExpired"
        :value="barangs" :loading="loading" :lazy="true" :paginator="true"
      ...
      >
        ...
        <Column header="#" style="width:45px;text-align:center">
          <template #body="{ index }"><span style="font-size:11px;color:#94a3b8">{{ (filters.currentPage-1)*filters.pageSize+index+1 }}</span></template>
        </Column>
        ...
        <Column header="SUB BARCODE" style="min-width:110px">
          <template #body="{ data }"><span class="mono">{{ data.SUB_BARCODE||'-' }}</span></template>
        </Column>
        ...
        <Column header="PERSEDIAAN" style="min-width:120px;text-align:right">
          <template #body="{ data }">
            <span class="mono">{{ data.PERSEDIAAN != null ? formatCurrency(data.PERSEDIAAN) : '-' }}</span>
          </template>
        </Column>
        <Column header="BATCH / NO.LOT" style="min-width:120px">
          <template #body="{ data }"><span class="mono">{{ data.BATCH_NUMBER||'-' }}</span></template>
        </Column>
        <Column header="EXPIRED" style="min-width:130px">
          <template #body="{ data }">
            <div v-if="data.TGL_EXPIRED" style="display:flex;flex-direction:column;gap:1px">
              <span class="mono fw-semibold" :style="{color: expiredUrgencyColor(data.TGL_EXPIRED)}">{{ formatDate(data.TGL_EXPIRED) }}</span>
              <span style="font-size:10px;font-weight:600" :style="{color: expiredUrgencyColor(data.TGL_EXPIRED)}">{{ daysUntil(data.TGL_EXPIRED) }} hari lagi</span>
            </div>
      ...
      >
        ...
        <Column field="GROUPING" header="GROUPING" style="min-width:140px">
          <template #body="{ data }"><span>{{ data.GROUPING||'-' }}</span></template>
        </Column>
          ...
          <div class="batch-expansion">
            ...
            <table v-else class="batch-table">
              <thead><tr>
                ...
                <th style="text-align:right">HARGA JUAL</th><th>BATCH / NO.LOT</th><th>EXPIRED</th><th>STATUS</th><th>AKSI</th>
              </tr></thead>
              <tbody>
                <tr v-for="batch in batchData[data.ID]" :key="batch.ID" :class="{'batch-row-arsip':batch.ARSIPKAN == 1}">
                  ...
                  <td style="text-align:right" class="mono">{{ formatCurrency(batch.HARGAJUAL) }}</td>
      ...
      <DataTable v-else
        :value="filteredGroups" :loading="loading" :lazy="true" :paginator="true"
        :rows="filters.pageSize" :totalRecords="totalBarangs"
        @page="onPageChange" @row-expand="onGroupExpand" v-model:expandedRows="expandedGroups"
        showGridlines rowHover scrollable scrollHeight="65vh" size="small"
        :rowsPerPageOptions="[20,50,100,500,1000]"
        paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
        class="barang-datatable"
      >
        ...
        <Column header="JUMLAH ITEM" style="min-width:110px;text-align:center">
          <template #body="{ data }"><span class="mono" style="color:#475569">{{ data.total_items }}</span></template>
        </Column>
                  ...
                  <tr :class="{'item-row-arsip': item.ARSIPKAN==='1'}">
                    <td>
                      <button class="item-expander" @click="toggleItemExpand(item.ID)">
                        <i :class="expandedItems[item.ID] ? 'pi pi-chevron-down' : 'pi pi-chevron-right'"></i>
                      </button>
                    </td>
                    <td>
                      <div style="display:flex;align-items:center;gap:6px">
                        <span style="font-size:12px;font-weight:600;color:#1e293b">{{ item.NAMA }}</span>
                        <Tag v-if="item.ARSIPKAN==='1'" value="Diarsipkan" severity="secondary" style="font-size:9px;padding:1px 5px" />
                      </div>
                      <div v-if="item.IDBARANG" class="mono" style="font-size:10px">{{ item.IDBARANG }}</div>
                    </td>
                    <td><span>{{ item.KATEGORI||'-' }}</span></td>
                    <td style="text-align:right">
                      <div style="display:inline-flex;align-items:center;gap:3px;justify-content:flex-end">
                        <span class="mono fw-semibold">{{ item.TOTAL_STOCK??'-' }}</span>
                        <i v-if="item.is_below_minimum" class="pi pi-exclamation-triangle" style="color:#dc2626;font-size:10px" v-tooltip.top="'Stok di bawah minimum'"></i>
                      </div>
                    </td>
                    <td style="text-align:right">
                      <span class="mono">{{ item.TOTAL_PERSEDIAAN != null ? formatCurrency(item.TOTAL_PERSEDIAAN) : '-' }}</span>
                    </td>
                    <td>
                      <span>{{ item.SATUAN_KECIL||'-' }}<span v-if="item.SATUAN_SEDANG" style="color:#6b7280"> / {{ item.SATUAN_SEDANG }}</span></span>
                    </td>
                    <td>
                      <div class="action-btns">
                        <Button icon="pi pi-eye" size="small" text rounded severity="info" v-tooltip.top="'Detail'" @click="viewBarang(item)" />
                        <Button icon="pi pi-pencil" size="small" text rounded severity="warn" v-tooltip.top="'Edit'" @click="openForm(item.ID)" />
                        <Button v-if="item.ARSIPKAN !== '1'" icon="pi pi-inbox" size="small" text rounded severity="warning" v-tooltip.top="'Arsipkan'" @click="arsipkanBarang(item.ID, item.NAMA)" />
                        <Button v-else icon="pi pi-check-circle" size="small" text rounded severity="success" v-tooltip.top="'Aktifkan'" @click="aktifkanBarang(item.ID, item.NAMA)" />
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr v-if="expandedItems[item.ID]" class="batch-sub-row">
                    <td colspan="7" style="padding:0">
                      <div class="batch-expansion" style="margin:0;border-top:none">
                        <div class="batch-expansion-header">
                          <i class="pi pi-list me-1"></i>Detail Batch — <strong>{{ item.NAMA }}</strong>
                        </div>
                        <div v-if="batchLoading[item.ID]" class="batch-loading"><i class="pi pi-spin pi-spinner me-2"></i>Memuat batch...</div>
                        <div v-else-if="!batchData[item.ID]?.length" class="batch-empty"><i class="pi pi-inbox me-1"></i>Tidak ada data batch di lokasi ini.</div>
                        <table v-else class="batch-table">
                          <thead><tr>
                            <th>SUB BARCODE</th>
                            <th style="text-align:right">QTY</th><th style="text-align:right">HARGA BELI</th>
                          ...
                          <tbody>
                            <tr v-for="batch in batchData[item.ID]" :key="batch.ID" :class="{'batch-row-arsip':batch.ARSIPKAN == 1}">
                              <td class="mono">{{ batch.SUB_BARCODE }}</td>
                              <td style="text-align:right" class="mono fw-semibold">{{ batch.QTY }}</td>
                              <td style="text-align:right" class="mono">{{ formatCurrency(batch.HARGA) }}</td>
                              <td style="text-align:right" class="mono">{{ batch.PERSEDIAAN != null ? formatCurrency(batch.PERSEDIAAN) : '-' }}</td>
                              ...
                              <td>
                                <Button v-if="batch.ARSIPKAN != 1" icon="pi pi-inbox" size="small" text rounded severity="warning" v-tooltip.top="'Arsipkan Batch'" @click="arsipkanBatch(batch.ID, batch.BATCH_NUMBER || batch.SUB_BARCODE, item.ID)" />
                                <Button v-else icon="pi pi-check-circle" size="small" text rounded severity="success" v-tooltip.top="'Aktifkan Batch'" @click="aktifkanBatch(batch.ID, batch.BATCH_NUMBER || batch.SUB_BARCODE, item.ID)" />
                              </td>
                            </tr>
                          </tbody>
                        </table>
    ...
    <!-- FORM MODAL -->
    <BarangFormModal v-model:visible="formModal.visible" bucket="OBAT_BMHP" :itemId="formModal.itemId" @saved="fetchBarang" />

    <!-- DETAIL DIALOG -->
    <Dialog v-model:visible="detailDialog" :header="selectedBarang?.NAMA||'Detail Obat/BMHP'" modal :style="{width:'580px',maxWidth:'95vw'}">
      <div v-if="selectedBarang" class="detail-body">
        ...
        <div class="detail-section-title mt-3"><i class="pi pi-heart me-1"></i>Informasi Farmasi</div>
        <div class="detail-grid">
          <div class="detail-row"><span class="detail-label">Sediaan</span><span class="detail-value">{{ selectedBarang.SEDIAAN||'-' }}</span></div>
          <div class="detail-row"><span class="detail-label">Rute Pemberian</span><span class="detail-value">{{ selectedBarang.RUTE||'-' }}</span></div>
          <div class="detail-row"><span class="detail-label">Zat Aktif</span><span class="detail-value" style="color:#0369a1">{{ selectedBarang.GROUPING||'-' }}</span></div>
        </div>
      </div>
      <template #footer>
...
const filters = ref({ search: '', ARSIPKAN: '0', JENIS: '', currentPage: 1, pageSize: 20, sortField: 'NAMA', sortOrder: 'ASC' })
const arsipOptions = [{ label: 'Semua Status', value: '' }, { label: 'Aktif', value: '0' }, { label: 'Diarsipkan', value: '1' }]
const jenisOptions = [{ label: 'Semua Jenis', value: '' }, { label: 'Obat-obatan', value: 'OBAT' }, { label: 'BMHP', value: 'BMHP' }]
...
const totalPersediaanDitampilkan = computed(() => {
  if (isGrouped.value) return groups.value.reduce((s, g) => s + (Number(g.total_persediaan) || 0), 0)
  ...
  try {
    ...
    if (showExpired.value) {
      ...
      params = {
        ...
        bucket: 'OBAT_BMHP', search: filters.value.search,
      ...
      params = {
        clientId: authStore.id_client, lokasiId: authStore.id_lokasi,
        bucket: 'OBAT_BMHP', grouped: isGrouped.value ? 1 : 0,
...
async function onGroupExpand(event) {
  ...
  try {
    const res = await axios.get(`${configStore.apiApotikUrl}/index.php/api/inventory/barang_group_items`, {
      params: { clientId: authStore.id_client, lokasiId: authStore.id_lokasi, bucket: 'OBAT_BMHP', grouping: key, mode: 1 },
    })
    groupItems.value[key] = res.data?.response || []
  } catch { groupItems.value[key] = [] }
  finally { groupItemsLoading.value[key] = false }
}
...
function onToggleGrouped() {
  filters.value.currentPage = 1
  expandedRows.value = []; expandedGroups.value = []
  groups.value = []; groupItems.value = {}; groupItemsLoading.value = {}; expandedItems.value = {}
  batchData.value = {}
  fetchBarang()
}
...
function resetFilter() {
  localSearch.value = ''
  expandAll.value = false; expandMode.value = 1; isGrouped.value = false; showBelowMin.value = false
  showExpired.value = false; expiredMonths.value = 1
  expandedRows.value = []; expandedGroups.value = []
  groups.value = []; groupItems.value = {}; groupItemsLoading.value = {}; expandedItems.value = {}
  batchData.value = {}
  filters.value = { search: '', ARSIPKAN: '0', JENIS: '', currentPage: 1, pageSize: 20, sortField: 'NAMA', sortOrder: 'ASC' }
  fetchBarang()
}
...
.barang-hero-title { margin:0;font-size:16px;font-weight:700;color:#38bdf8;letter-spacing:.5px; }
.barang-hero-sub { margin:0;font-size:11px;color:rgba(255,255,255,.65); }
.barang-stat-strip { display:flex;align-items:center;height:40px;background:#fff;border:1px solid #e2e8f0;border-top:3px solid #0369a1;border-radius:0 0 6px 6px;padding:0 6px;overflow:hidden;margin-bottom:6px; }
...
.barang-filter-bar { display:flex;align-items:center;flex-wrap:wrap;gap:0;padding:10px 16px;background:#fff;border:1px solid #e2e8f0;border-top:3px solid #0369a1;border-radius:8px;margin-bottom:6px;box-shadow:0 1px 4px rgba(0,0,0,.05); }
...
.barang-filter-label i { color:#0369a1;font-size:9px; }
...
.filter-chip:hover:not(:disabled) { border-color:#bae6fd;background:#eff6ff;color:#0369a1; }
.filter-chip.active { border-color:#0369a1 !important;background:#e0f2fe !important;color:#0369a1 !important;font-weight:600 !important; }
...
.filter-chip.active-expired { border-color:#d97706 !important;background:#fffbeb !important;color:#d97706 !important;font-weight:600 !important; }
...
.barang-table-wrap { background:#fff;border:1px solid #e2e8f0;border-top:3px solid #0369a1;border-radius:6px;overflow:hidden; }
:deep(.barang-datatable .p-datatable-thead > tr > th) { background:#eff6ff !important;color:#1d4ed8 !important;font-size:11px !important;font-weight:700 !important;text-transform:uppercase !important;letter-spacing:.5px !important;padding:.25rem .6rem !important;white-space:nowrap; }
...
:deep(.barang-datatable .p-datatable-tbody > tr:hover > td) { background:#eff6ff !important; }
...
.detail-section-title { font-size:11.5px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#1d4ed8;padding:4px 0;border-bottom:1px solid #dbeafe;margin-bottom:8px; }
...
.btn-primary-barang { background:#0369a1 !important;border-color:#0369a1 !important;color:#fff !important; }
.btn-primary-barang:hover { background:#0284c7 !important;border-color:#0284c7 !important; }
.round-button2 { border-radius:6px;font-size:12px;padding:6px 12px; }
.group-items-expansion { padding:0; }
.items-table { width:100%;border-collapse:collapse;font-size:12px; }
.items-table thead tr { background:#eff6ff; }
.items-table thead th { padding:5px 8px;text-align:left;font-size:10.5px;font-weight:700;color:#1d4ed8;text-transform:uppercase;letter-spacing:.4px;white-space:nowrap;border-bottom:2px solid #bfdbfe; }
.items-table tbody tr { border-bottom:1px solid #e2e8f0; }
.items-table tbody tr:not(.batch-sub-row):hover { background:#f8fafc; }
.items-table tbody td { padding:1px 6px;vertical-align:middle; }
.item-row-arsip { opacity:.55; }
.item-expander { background:none;border:none;cursor:pointer;padding:2px 5px;color:#64748b;font-size:11px;border-radius:3px;line-height:1; }
.item-expander:hover { background:#dbeafe;color:#1d4ed8; }
.batch-sub-row > td { background:#f1f5f9 !important;padding:0 !important; }
...
.batch-table thead tr { background:#dbeafe; }
.batch-table thead th { padding:4px 8px;text-align:left;font-size:10.5px;font-weight:700;color:#1d4ed8;text-transform:uppercase;letter-spacing:.4px;white-space:nowrap;border-bottom:1px solid #bfdbfe; }
.batch-table tbody tr { border-bottom:1px solid #e2e8f0; }
.batch-table tbody tr:hover { background:#eff6ff; }
